...
describe('authController', () => {
  ...
  describe('handlePasswordResetRequest', () => {
    it('should send reset email and url on success', async () => {
      ...
      authService.requestPasswordReset.mockResolvedValue({ resetUrl: 'http://reset.url' });
    ...
    it('should return 500 if email sending fails', async () => {
      ...
      authService.requestPasswordReset.mockRejectedValue(new Error('Failed to send reset email'));
    ...
    it('should return 500 for other errors', async () => {
      ...
      authService.requestPasswordReset.mockRejectedValue(new Error('Some other error'));

      await authController.handlePasswordResetRequest(req, res);

      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ error: 'Error processing password reset request' });
    });
  });
  ...
  describe('handlePasswordReset', () => {
    it('should return success message on valid reset', async () => {
      ...
      expect(authService.resetPassword).toHaveBeenCalledWith('token', 'newpass');
  ...
  describe('githubAuth', () => {
    ...
    it('should return 401 for bad credentials', async () => {
      ...
      authService.handleGitHubAuth.mockRejectedValue(new Error('Bad credentials'));
    ...
    it('should return 500 for other errors', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ error: 'Error during authentication' });
    });
  });
});